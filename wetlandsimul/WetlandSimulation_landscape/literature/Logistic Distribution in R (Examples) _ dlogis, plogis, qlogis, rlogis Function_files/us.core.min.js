...
this.minWidth=Math.max(480,minContentWidth+minTabWidth+1);this.tabHeights=[];this.tabTops=[];for(var index=0;index<this.tabs.length;index ++){this.tabHeights.push(this.tabs[index].outerHeight(!0));this.tabTops.push(index?(this.tabTops[index-1]+this.tabHeights[index-1]):0)}}else{this.tabWidths=[];this.tabLefts=[];if(this.isStretched){this.curTabWidth=this.tabs[0].outerWidth(!0)}
this.$container.addClass('measure');for(var index=0;index<this.tabs.length;index ++){this.tabWidths.push(this.tabs[index].outerWidth(!0));if(this.isStretched){this.tabLefts.push(index?(this.tabLefts[index-1]+this.curTabWidth):0)}else{this.tabLefts.push(index?(this.tabLefts[index-1]+this.tabWidths[index-1]):0)}}
this.$container.removeClass('measure');if(this.basicLayout=='hor'){this.minWidth=this.tabWidths.reduce(function(pv,cv){return pv+cv},0)}}},barPosition:function(index){if(this.curLayout==='hor'){return{left:this.tabLefts[index],width:(this.isStretched)?this.curTabWidth:this.tabWidths[index]}}
